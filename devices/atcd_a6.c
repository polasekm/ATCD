...
uint16_t atcd_proc_step()
{
  ...
  switch(atcd.proc_step)
  {
    ...
    case ATCD_SB_GPRS_INIT:
      ...
      //pokud neni registrovan, nektere modemy se nasledne mohou zamyslet na
      // desitky sekudn...
    ...
    case ATCD_SB_GPRS_INIT + 6:
      ...
      //Modem se chova sahle, pokud se neudela CGDCONT, vytuhne pri CIICR
      //Rozpor s dokumentaci...

      strcpy(atcd.at_cmd_buff, "AT+CGDCONT=1,\"IP\",\"");
      strcat(atcd.at_cmd_buff, atcd.gprs.apn);
      strcat(atcd.at_cmd_buff, "\"\r\n");
