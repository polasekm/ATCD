...
void atcd_gprs_init_seq()
{
  ...
  if(atcd.gprs.at_cmd.state == ATCD_ATC_STATE_FREE)
  {
    ...
    switch(atcd.gprs.at_cmd_seq)
    {
      ...
      case 1: cmd = "AT^SICS=0,\"conType\",\"GPRS0\"\r\n";      break;
      case 2: cmd = "AT^SICS=0,\"inactTO\",\"20\"\r\n";         break;
      case 3: cmd = "AT^SICS=0,\"apn\",\"internet\"\r\n";       break;
      case 4: cmd = "AT^SICS=0,\"user\",\"\"\r\n";              break;
      case 5: cmd = "AT^SICS=0,\"passwd\",\"\"\r\n";            break;
      //case 4: cmd = "AT^SICO=0"\r\n";                          break;
  ...
  if(atcd.gprs.at_cmd.state == ATCD_ATC_STATE_DONE)
  {
    ...
    else 
    {
      ...
      if(atcd.gprs.at_cmd.result == ATCD_ATC_RESULT_OK)
      {
        ...
        {
          atcd_dbg_inf("GPRS: STAT: GPRS je odpojeno.\r\n");
        }
      }
      else 
      {
...
void atcd_conn_open_seq(atcd_conn_t *conn)
{
  ...
  // Zpracovani vysledku posledne volaneho AT prikazu
  //--------------------------------------------------
  if(conn->at_cmd.state == ATCD_ATC_STATE_DONE)
  {
    if(conn->at_cmd_seq == 0) conn->at_cmd.state = ATCD_ATC_STATE_FREE;
    else
    {
      if(conn->at_cmd.result == ATCD_ATC_RESULT_OK)
      {
        atcd_dbg_inf("CONN: INIT: AT prikaz byl dokoncen.\r\n");
        // ... Zpracovat odpoved...
        conn->at_cmd_seq++;
      }
      else
      {
        if(conn->at_cmd_seq == 1) conn->at_cmd_seq++;
        else
        {
          atcd_dbg_warn("CONN: INIT: Pri zpracovani AT prikazu doslo k chybe.\r\n");
          atcd_dbg_warn("CONN: Pokus o otevreni spojeni skoncil chybou!\r\n");
          atcd.conns.conn[conn->num]  = NULL;
  ...
  if(conn->at_cmd.state == ATCD_ATC_STATE_FREE)
  {
    ...
    switch(conn->at_cmd_seq)
    {
      case 0:
        atcd_dbg_inf("CONN: Otevitam spojeni.\r\n");
        conn->at_cmd_seq = 1;

      case 1: sprintf(conn->at_cmd_buff, "AT^SISC=%u\r\n", conn->num);                        break;
      case 2: sprintf(conn->at_cmd_buff, "AT^SISS=%u,\"srvType\",\"Socket\"\r\n", conn->num); break;
      case 3: sprintf(conn->at_cmd_buff, "AT^SISS=%u,\"conId\",\"0\"\r\n", conn->num);        break;
      case 4:
        //volba tcp/udp
        //if(conn->protocol == ATCD_CONN_T_TCP)
        //else if(conn->protocol == ATCD_CONN_T_UDP)
        ...
        sprintf(conn->at_cmd_buff, "AT^SISS=%u,\"address\",\"socktcp://%s:%u\"\r\n", conn->num, conn->host, conn->port);
        break;
      case 5: sprintf(conn->at_cmd_buff, "AT^SISO=%u\r\n", conn->num);                        break;

      case 6:
        atcd_dbg_inf("CONN: INIT: Init sekvence dokoncena - cekam na spojeni.\r\n");
        conn->state        = ATCD_CONN_STATE_OPENING;
        conn->at_cmd.state = ATCD_ATC_STATE_FREE;    // zbytecne, musi byt s ohledem na podminku
        conn->timer        = atcd_get_ms();
        conn->at_cmd_seq   = 0;
        // asi volat call back pokud je nastaven?
        return;
      ...
      default:
        atcd_dbg_err("CONN: INIT: Sekvence ma neplatne cislo kroku.\r\n");
        atcd_dbg_warn("CONN: Pokus o otevreni spojeni skoncil chybou!\r\n");
        atcd.conns.conn[conn->num]  = NULL;
    ...
    atcd_dbg_inf("CONN: INIT: Odesilam AT prikaz ze sekvence.\r\n");
